...
+(NSArray*)GetDataFromJsonParser:(NSURL *)urlstring
{
    ...
    if (LogValue) {
        [self Debug:[NSString stringWithContentsOfURL:urlstring encoding:NSUTF8StringEncoding error:nil] Response:response];
    }
...
+ (id) ExecuteWebService:(NSString*)urlParameters
{
	id webResult=nil;
...
+(void)Debug:(NSString*)url Response:(NSData*)response
{
    if (LogValue) {
        NSString *strRespnse = [[NSString alloc] initWithData:response encoding:NSUTF8StringEncoding];
        NSLog(@"Url: %@\nResponse: %@",url,strRespnse);
    }
}
